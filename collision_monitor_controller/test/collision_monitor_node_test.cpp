...
#include "collision_monitor_controller/collision_monitor_node.hpp"
...
enum PolygonType { POLYGON_UNKNOWN = 0, POLYGON = 1, CIRCLE = 2, VELOCITY_POLYGON = 3 };
...
class CollisionMonitorWrapper : public collision_monitor_controller::CollisionMonitor
{
public:
  void start()
  {
  ...
  {
    for (std::shared_ptr<collision_monitor_controller::Source> source : sources_) {
      std::vector<collision_monitor_controller::Point> collision_points;
...
public:
  ...
  void addPolygonVelocitySubPolygon(
    const std::string & polygon_name, const std::string & sub_polygon_name, const double linear_min,
    const double linear_max, const double theta_min, const double theta_max, const double size);
  ...
  void publishPointCloudWithHeight(
    const double dist, const double height, const rclcpp::Time & stamp);
  ...
  const double dist, const double height, const rclcpp::Time & stamp)
{
  ...
  modifier.setPointCloud2Fields(
    ...
    "z", 1, sensor_msgs::msg::PointField::FLOAT32, "height", 1,
