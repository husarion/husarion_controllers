...
public:
  ...
  : SimpleActionServer(
      ...
      execute_callback, completion_callback, server_timeout, spin_thread, options, realtime)
  {
  }
    ...
    std::shared_ptr<const typename ActionT::Goal> /*goal*/)
  {
    ...
    if (!server_active_) {
      RCLCPP_INFO(
        node_logging_interface_->get_logger(), "Action server is inactive. Rejecting the goal.");
