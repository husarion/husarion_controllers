...
static const std::vector<double> FORWARD_POLYGON{0.5, 0.5, 0.5, -0.5, 0.0, -0.5, 0.0, 0.5};
static const std::vector<double> BACKWARD_POLYGON{0.0, 0.5, 0.0, -0.5, -0.5, -0.5, -0.5, 0.5};
static const std::vector<double> LEFT_POLYGON{0.5, 0.5, 0.5, 0.0, 0.0, 0.0, 0.0, -0.5};
static const std::vector<double> RIGHT_POLYGON{0.5, 0.0, 0.5, -0.5, -0.5, -0.5, 0.0, 0.0};
static const char FORWARD_POLYGON_STR[]{"[[0.5, 0.5], [0.5, -0.5], [0.0, -0.5], [0.0, 0.5]]"};
static const char BACKWARD_POLYGON_STR[]{"[[0.0, 0.5], [0.0, -0.5], [-0.5, -0.5], [-0.5, 0.5]]"};
static const char LEFT_POLYGON_STR[]{"[[0.5, 0.5], [0.5, 0.0], [0.0, 0.0], [0.0, -0.5]]"};
static const char RIGHT_POLYGON_STR[]{"[[0.5, 0.0], [0.5, -0.5], [-0.5, -0.5], [0.0, 0.0]]"};
...
class VelocityPolygonWrapper : public collision_monitor_controller::VelocityPolygon
{
public:
  ...
  : collision_monitor_controller::VelocityPolygon(
      node, polygon_name, tf_buffer, base_frame_id, transform_tolerance)
  {
  }

  double isHolonomic() const { return holonomic_; }

  double isVisualize() const { return visualize_; }

  std::vector<SubPolygonParameter> getSubPolygons() { return sub_polygons_; }
};  // VelocityPolygonWrapper
...
protected:
  ...
  void addPolygonVelocitySubPolygon(
    const std::string & sub_polygon_name, const double linear_min, const double linear_max,
    const double theta_min, const double theta_max, const double direction_end_angle,
    const double direction_start_angle, const std::string & polygon_points,
  ...
  bool waitPolygon(
    const std::chrono::nanoseconds & timeout,
    std::vector<collision_monitor_controller::Point> & poly);
...
void Tester::setCommonParameters(const std::string & polygon_name, const std::string & action_type)
{
  ...
  test_node_->declare_parameter(polygon_name + ".min_points", rclcpp::ParameterValue(MIN_POINTS));
  test_node_->set_parameter(rclcpp::Parameter(polygon_name + ".min_points", MIN_POINTS));
...
void Tester::setVelocityPolygonParameters(const bool is_holonomic)
{
  ...
  if (is_holonomic) {
    ...
    addPolygonVelocitySubPolygon(
      ...
      BACKWARD_POLYGON_STR, is_holonomic);
    addPolygonVelocitySubPolygon(
      SUB_POLYGON_LEFT_NAME, -0.5, 0.5, -1.0, 1.0, M_PI_4, 0.75 * M_PI, LEFT_POLYGON_STR,
      is_holonomic);
    addPolygonVelocitySubPolygon(
      SUB_POLYGON_RIGHT_NAME, -0.5, 0.5, -1.0, 1.0, -0.75 * M_PI, -M_PI_4, RIGHT_POLYGON_STR,
      is_holonomic);

    velocity_polygons = {
      SUB_POLYGON_FORWARD_NAME, SUB_POLYGON_BACKWARD_NAME, SUB_POLYGON_LEFT_NAME,
      SUB_POLYGON_RIGHT_NAME};
  } else {
    // draw forward and backward polygon
    addPolygonVelocitySubPolygon(
      SUB_POLYGON_FORWARD_NAME, 0.0, 0.5, -1.0, 1.0, 0.0, 0.0, FORWARD_POLYGON_STR, is_holonomic);
    addPolygonVelocitySubPolygon(
...
void Tester::addPolygonVelocitySubPolygon(
  const std::string & sub_polygon_name, const double linear_min, const double linear_max,
  const double theta_min, const double theta_max, const double direction_start_angle,
  const double direction_end_angle, const std::string & polygon_points, const bool is_holonomic)
{
  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".points",
    rclcpp::ParameterValue(polygon_points));
  test_node_->set_parameter(rclcpp::Parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".points", polygon_points));

  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_min",
    rclcpp::ParameterValue(linear_min));
  test_node_->set_parameter(rclcpp::Parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_min", linear_min));

  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_max",
    rclcpp::ParameterValue(linear_max));
  test_node_->set_parameter(rclcpp::Parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".linear_max", linear_max));

  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_min",
    rclcpp::ParameterValue(theta_min));
  test_node_->set_parameter(rclcpp::Parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_min", theta_min));

  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_max",
    rclcpp::ParameterValue(theta_max));
  test_node_->set_parameter(rclcpp::Parameter(
    std::string(POLYGON_NAME) + "." + sub_polygon_name + ".theta_max", theta_max));
  ...
  if (is_holonomic) {
    ...
    test_node_->declare_parameter(
      std::string(POLYGON_NAME) + "." + sub_polygon_name + ".direction_start_angle",
...
bool Tester::waitPolygon(
  const std::chrono::nanoseconds & timeout, std::vector<collision_monitor_controller::Point> & poly)
{
...
TEST_F(Tester, testVelocityPolygonGetStopParameters)
{
  ...
  EXPECT_EQ(velocity_polygon_->getActionType(), collision_monitor_controller::STOP);
...
TEST_F(Tester, testVelocityPolygonGetSlowdownParameters)
{
  ...
  // Check that common parameters set correctly
  EXPECT_EQ(velocity_polygon_->getName(), POLYGON_NAME);
  EXPECT_EQ(velocity_polygon_->getActionType(), collision_monitor_controller::SLOWDOWN);
  EXPECT_EQ(velocity_polygon_->getMinPoints(), MIN_POINTS);
  EXPECT_EQ(velocity_polygon_->isVisualize(), true);
}
...
TEST_F(Tester, testVelocityPolygonOutOfRangeVelocity)
{
  ...
  collision_monitor_controller::Velocity vel{0.6, 0.0, 0.0};
...
TEST_F(Tester, testVelocityPolygonVelocitySwitching)
{
  ...
  collision_monitor_controller::Velocity vel{0.3, 0.0, 0.0};
  velocity_polygon_->updatePolygon(vel);
  ASSERT_TRUE(waitPolygon(500ms, poly));
  ASSERT_EQ(poly.size(), 4u);
