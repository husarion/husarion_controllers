...
TEST(KinematicsTest, testTransformPoints)
{
  ...
  const collision_monitor_controller::Pose tf{2.0, 1.0, M_PI / 6.0};
  ...
  collision_monitor_controller::transformPoints(tf, points);
...
TEST(KinematicsTest, testProjectState)
{
  ...
  collision_monitor_controller::Pose pose{2.0, 1.0, M_PI / 4.0};
  // Initial velocity of robot
  collision_monitor_controller::Velocity vel{0.0, 1.0, M_PI / 4.0};
  const double dt = 1.0;

  // Moving robot and rotating velocity
  collision_monitor_controller::projectState(dt, pose, vel);
