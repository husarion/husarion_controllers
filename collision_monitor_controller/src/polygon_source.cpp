...
PolygonSource::~PolygonSource() { data_sub_.reset(); }
...
void PolygonSource::configure()
{
  ...
  data_sub_ = node->create_subscription<geometry_msgs::msg::PolygonInstanceStamped>(
    source_topic, qos, std::bind(&PolygonSource::dataCallback, this, std::placeholders::_1));
}

bool PolygonSource::getData(const rclcpp::Time & curr_time, std::vector<Point> & data)
{
  ...
  for (const auto & polygon_instance : data_) {
    if (base_shift_correction_) {
      ...
      if (!nav2_util::getTransform(
            polygon_instance.header.frame_id, polygon_instance.header.stamp, base_frame_id_,
            curr_time, global_frame_id_, transform_tolerance_, tf_buffer_, tf_transform)) {
        return false;
      }
    } else {
      ...
      // frames.
      if (!nav2_util::getTransform(
            polygon_instance.header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_,
            tf_transform)) {
        return false;
      }
    }
  ...
  const geometry_msgs::msg::PolygonStamped & polygon, std::vector<Point> & data) const
{
  ...
  for (size_t i = 0; i < polygon.polygon.points.size(); ++i) {
    ...
    int num_points_in_segment = std::max(
      static_cast<int>(std::ceil(segment_length / sampling_distance_)), 1);
