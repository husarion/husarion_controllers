...
class ScanWrapper : public collision_monitor_controller::Scan
{
public:
  ScanWrapper(
  ...
  : collision_monitor_controller::Scan(
...
public:
  PointCloudWrapper(
    ...
    const std::string & global_frame_id, const tf2::Duration & transform_tolerance,
    const rclcpp::Duration & data_timeout, const bool base_shift_correction)
  : collision_monitor_controller::PointCloud(
      node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance,
      data_timeout, base_shift_correction)
  {
  }
...
public:
  RangeWrapper(
    ...
    const std::string & global_frame_id, const tf2::Duration & transform_tolerance,
    const rclcpp::Duration & data_timeout, const bool base_shift_correction)
  : collision_monitor_controller::Range(
      node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance,
      data_timeout, base_shift_correction)
  {
  }
...
public:
  PolygonWrapper(
    ...
    const std::string & global_frame_id, const tf2::Duration & transform_tolerance,
    const rclcpp::Duration & data_timeout, const bool base_shift_correction)
  : collision_monitor_controller::PolygonSource(
      node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance,
      data_timeout, base_shift_correction)
  {
  }

  bool dataReceived() const { return data_.size() > 0; }
};  // PolygonWrapper
...
protected:
  ...
  void checkScan(const std::vector<collision_monitor_controller::Point> & data);
  void checkPointCloud(const std::vector<collision_monitor_controller::Point> & data);
  void checkRange(const std::vector<collision_monitor_controller::Point> & data);
  void checkPolygon(const std::vector<collision_monitor_controller::Point> & data);
...
void Tester::createSources(const bool base_shift_correction)
{
  ...
  test_node_->set_parameter(rclcpp::Parameter(std::string(POLYGON_NAME) + ".topic", POLYGON_TOPIC));

  test_node_->declare_parameter(
    std::string(POLYGON_NAME) + ".sampling_distance", rclcpp::ParameterValue(0.1));

  polygon_ = std::make_shared<PolygonWrapper>(
    test_node_, POLYGON_NAME, tf_buffer_, BASE_FRAME_ID, GLOBAL_FRAME_ID, TRANSFORM_TOLERANCE,
    DATA_TIMEOUT, base_shift_correction);
  polygon_->configure();
}
...
void Tester::checkScan(const std::vector<collision_monitor_controller::Point> & data)
{
...
void Tester::checkPointCloud(const std::vector<collision_monitor_controller::Point> & data)
{
...
void Tester::checkRange(const std::vector<collision_monitor_controller::Point> & data)
{
...
void Tester::checkPolygon(const std::vector<collision_monitor_controller::Point> & data)
{
...
TEST_F(Tester, testGetData)
{
  ...
  // Check Scan data
  std::vector<collision_monitor_controller::Point> data;
...
TEST_F(Tester, testPointCloudMinRange)
{
  ...
  test_node_->set_parameter(rclcpp::Parameter(std::string(POINTCLOUD_NAME) + ".min_range", 0.16));
