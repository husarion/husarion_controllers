...
#include "collision_monitor_controller/kinematics.hpp"
...
bool Polygon::configure()
{
  ...
  if (!footprint_topic.empty()) {
    RCLCPP_INFO(
      logger_, "[%s]: Making footprint subscriber on %s topic", polygon_name_.c_str(),
...
bool Polygon::getEnabled() const { return enabled_; }
...
std::vector<std::string> Polygon::getSourcesNames() const { return sources_names_; }
...
void Polygon::updatePolygon(const Velocity & /*cmd_vel_in*/)
{
  if (footprint_sub_ != nullptr) {
    ...
    if (!nav2_util::getTransform(
          polygon_.header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_,
          tf_transform)) {
      return;
    }
...
bool Polygon::getCommonParameters(
  std::string & polygon_sub_topic, std::string & polygon_pub_topic, std::string & footprint_topic,
  ...
  try {
    ...
    if (use_dynamic_sub_topic) {
      if (action_type_ != APPROACH) {
        ...
        polygon_sub_topic = node->get_parameter(polygon_name_ + ".polygon_sub_topic").as_string();
      } else {
    ...
    for (auto source_name : sources_names_) {
      if (
        std::find(observation_sources.begin(), observation_sources.end(), source_name) ==
        observation_sources.end()) {
        RCLCPP_ERROR_STREAM(
          logger_, "Observation source ["
                     << source_name << "] configured for polygon [" << getName()
  ...
  std::string & polygon_sub_topic, std::string & polygon_pub_topic, std::string & footprint_topic)
{
  ...
  try {
    ...
    std::string poly_string = node->get_parameter(polygon_name_ + ".points").as_string();

    use_dynamic_sub = !getPolygonFromString(poly_string, poly_);
  } catch (const rclcpp::exceptions::ParameterUninitializedException &) {
    RCLCPP_INFO(
      logger_, "[%s]: Polygon points are not defined. Using dynamic subscription instead.",
      polygon_name_.c_str());
  }
...
void Polygon::createSubscription(std::string & polygon_sub_topic)
{
  ...
  if (!polygon_sub_topic.empty()) {
    RCLCPP_INFO(
      logger_, "[%s]: Subscribing on %s topic for polygon", polygon_name_.c_str(),
...
void Polygon::updatePolygon(geometry_msgs::msg::PolygonStamped::ConstSharedPtr msg)
{
  ...
  if (new_size < 3) {
    RCLCPP_ERROR(logger_, "[%s]: Polygon should have at least 3 points", polygon_name_.c_str());
    return;
  }
  ...
  tf2::Stamped<tf2::Transform> tf_transform;
  if (!nav2_util::getTransform(
        msg->header.frame_id, base_frame_id_, transform_tolerance_, tf_buffer_, tf_transform)) {
    return;
  }
...
rcl_interfaces::msg::SetParametersResult Polygon::dynamicParametersCallback(
...
{
  RCLCPP_INFO(logger_, "[%s]: Polygon shape update has been arrived", polygon_name_.c_str());
  updatePolygon(msg);
}
...
inline bool Polygon::isPointInside(const Point & point) const
{
  ...
  for (j = 0; j < poly_size; j++) {
    ...
    if ((point.y <= poly_[i].y) == (point.y > poly_[j].y)) {
      ...
      const double x_inter = poly_[i].x + (point.y - poly_[i].y) * (poly_[j].x - poly_[i].x) /
                                            (poly_[j].y - poly_[i].y);
...
bool Polygon::getPolygonFromString(std::string & poly_string, std::vector<Point> & polygon)
{
  std::string error;
  ...
  if (error != "") {
    RCLCPP_ERROR(
      logger_, "Error parsing polygon parameter %s: '%s'", poly_string.c_str(), error.c_str());
    return false;
  }

  // Check for minimum 4 points
  if (vvf.size() <= 3) {
    RCLCPP_ERROR(logger_, "Polygon must have at least three points.");
    return false;
  }
