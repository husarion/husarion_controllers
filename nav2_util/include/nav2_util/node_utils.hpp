...
void declare_parameter_if_not_declared(
  NodeT node, const std::string & parameter_name, const rclcpp::ParameterValue & default_value,
...
template <typename NodeT>
void declare_parameter_if_not_declared(
  NodeT node, const std::string & parameter_name, const rclcpp::ParameterType & param_type,
  const ParameterDescriptor & parameter_descriptor = ParameterDescriptor())
{
  if (!node->has_parameter(parameter_name)) {
 ...
 * \param[in] warn_if_no_override If true, prints a warning whenever the parameter override is
 * missing
 * \param[in] strict_param_loading If true, throws an InvalidParameterValueException if the
 * parameter override is missing
...
ParamType declare_or_get_parameter(
  NodeT node, const std::string & parameter_name, const ParamType & default_value,
  const ParameterDescriptor & parameter_descriptor = ParameterDescriptor())
{
  ...
  return declare_or_get_parameter(
    node->get_logger(), node->get_node_parameters_interface(), parameter_name, default_value,
    warn_if_no_override, strict_param_loading, parameter_descriptor);
}
